...
mts.woody.df <- mts.woody.df %>% select(site, agaaus:weisil)
...
hira.e <- hira.range$elev.lims[hira.range$elev.lims$spp %in% lowland.spp, ]
all.e <- all_sites.range$elev.lims[all_sites.range$elev.lims$spp %in% lowland.spp, ]
...
hira.f.e <- hira.ferns.range$elev.lims[hira.ferns.range$elev.lims$spp %in% ferns.all_sites, ]
...
hira.range <- (hira.wdy.rg | hira.fern.rg) +
  plot_annotation(tag_levels = "a")


all.range <- (all.wdy.rg | all.fern.rg) +
  plot_annotation(tag_levels = "a")
